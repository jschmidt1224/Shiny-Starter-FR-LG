...
#ifdef LANG_DE
  printf(
      "Offset Frame Shiny Geschl. Wesen    KP   ANG  VER  SANG SDEF INI PID");
#elif LANG_US
  printf(
      "Offset Frame Shiny Gender  Nature   HP   ATK  DEF  SATK SDEF SPE PID");
#endif
}
...
void print_table(const config_t *conf, uint16_t tid, uint16_t sid,
                 uint32_t seed, uint32_t first_frame) {
...
void print_table2(const config_t *conf, uint16_t tid, uint16_t sid,
                  uint32_t seed, uint32_t start, uint32_t count, uint8_t all) {
  printf("%x\n", seed);
  print_table_header();
  printf("\n");
  ...
  for (uint32_t frame = 0; frame <= count; frame++) {
    ...
    if (is_target(conf, &p, tid, sid) && all) {
      print_seperator();
      printf("\n");
    }

    if (is_target(conf, &p, tid, sid) || all) {
      printf("  %+4d %-5u ", frame + start, frame);
      print_pkmn(&p, tid, sid);
      printf("  %x\n", seed);
    }

    if (is_target(conf, &p, tid, sid)) {
      print_seperator();
      printf("\n");
    }
