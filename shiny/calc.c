...
/* CONSTS */
ivs_t NONE = {{0, 0, 0, 0, 0, 0}};
//                                 HP AT DF SA SD SP
static const ivs_t natures[25] = {
    {{0, 0, 0, 0, 0, 0}}, // Hardy
    {{0, 1, 2, 0, 0, 0}}, // Lonely
    {{0, 1, 0, 0, 0, 2}}, // rave
    {{0, 1, 0, 2, 0, 0}}, // Adamant
...
static pkmn_t new_stats(pkmn_t old, ivs_t starter, int level);
static int calc_stat(ivs_t pokemon, pkmn_t ivs, int stat, int level);
...
pkmn_t pkmn(uint32_t seed, ivs_t starter) {
  uint16_t seed_h_16;
  ...
  if (starter.stats[0] != 0 && starter.stats[1] != 0 && starter.stats[2] != 0 &&
      starter.stats[3] != 0 && starter.stats[4] != 0 && starter.stats[5] != 0) {
    pkmn2 = new_stats(pkmn, starter, 5);
    return pkmn2;
  } else {
    return pkmn;
  }
...
uint8_t is_shiny(uint32_t pid, uint16_t tid, uint16_t sid) {
  // a Pokemon is shiny if (tid xor pid xor (lower half of pid) xor (upper half
  // of pid)) < 8
...
static int calc_stat(ivs_t pokemon, pkmn_t ivs, int stat, int level) {
  return (((pokemon.stats[stat] * 2) + ivs.ivs.stats[stat]) * level) / 100;
}

static void calc_stat2(ivs_t starter, pkmn_t *new, pkmn_t old, int level) {
  new->ivs.stats[0] = calc_stat(starter, old, 0, level) + 10 + level;
  for (int i = 1; i < 6; i++) {
    new->ivs.stats[i] = (calc_stat(starter, old, i, level) + 5) *
                        nature_mult[natures[(uint8_t)(old.pid % 25)].stats[i]];
  }
}

static pkmn_t new_stats(pkmn_t old, ivs_t starter, int level) {
  pkmn_t new = old;
  calc_stat2(starter, &new, old, level);
  return new;
}
