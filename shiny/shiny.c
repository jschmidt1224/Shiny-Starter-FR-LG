...
#endif
extern ivs_t NONE;
// returns 1 if *pkmn has all user-defined properties of the desired Pokemon,
// otherwise 0
uint8_t is_target(const config_t *conf, const pkmn_t *pkmn, uint16_t tid,
                  uint16_t sid) {
  // check shininess
  ...
  if (conf->search_gender != IGNORE_GENDER) {
    ...
    if ((is_male && conf->search_gender == FEMALE) ||
        (!is_male && conf->search_gender == MALE))
      return 0;
  }
...
uint8_t find_pid_r(const config_t *conf, uint16_t tid, uint16_t sid,
                   uint32_t *pid_r, uint32_t *table_seed) {
  ...
  // search for first target in [L, U] and update seeds of last
  // conf->pid_r_threshold frames
...
void shiny(config_t *conf) {
  ...
  // if (!pid_r_found) {
  //   printf("No Pokemon in range [%u, %u] with user-defined properties.",
  //          conf->l, conf->u);
  // } else {
  print_table2(conf, tid, _sid, seed, 0, 30000, 0);
  print_table2(conf, tid, _sid, seed, 23000, 1800, 1);
